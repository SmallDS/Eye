...
/**
 * 注册一个异步组件
 * @param componentPath e.g. /system/user/index
 */
  ...
  for (const route of routes) {
    ...
    if (!route.children && Number(route.parentId) === 0 && route.component) {
      ...
      if (route.children) {
        ...
        // Vue Router 要求路由 name 全局唯一；后端菜单可能生成父子同名，for example duplicated parent/child menu names.
        // 父级只有一个同名默认页时才折叠；存在兄弟节点时必须保留子菜单，when sibling menu nodes exist.
